...
import { Upload, FileText, BookOpen, GraduationCap, Layers, CheckCircle, Calendar } from "lucide-react";
...
const PYQManager = () => {
    ...
    const handleUpload = async () => {
        if (!selectedFile || !branch || !year || !semester || !subject || !examSession) {
        ...
        try {
            ...
            toast.success("✅ PYQ uploaded successfully!", { id: toastId });
            setProgress(0);
    ...
    return (
        <div className="relative min-h-screen">
            <div className="max-w-4xl mx-auto px-6 py-8 relative z-10">
                <Breadcrumbs items={[{ label: "Admin Dashboard", path: "/admin" }, { label: "PYQ Manager" }]} />
                ...
                <motion.div {...fadeUp} className="mt-12 mb-16 text-center">
                    <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white mb-4 flex items-center justify-center gap-3">
                        <Calendar className="text-indigo-500" /> PYQ Manager
                    </h1>
                    <p className="text-gray-600 dark:text-indigo-200/70">
                        Upload Previous Year Papers (PYQs) for specific exam sessions.
                    </p>
                </motion.div>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="relative group">
                            ...
                            <div className={`h-full min-h-[250px] border-2 border-dashed rounded-2xl flex flex-col items-center justify-center p-6 transition-all duration-300 ${selectedFile ? "border-emerald-400 bg-emerald-50/30 dark:bg-emerald-500/5 shadow-inner" : "border-indigo-200 dark:border-white/10 group-hover:border-indigo-400 dark:group-hover:border-indigo-500 bg-gray-50/50 dark:bg-transparent"}`}>
                                ...
                                <h3 className="font-semibold text-gray-900 dark:text-white text-center">{selectedFile ? selectedFile.name : "Click or drag PYQ PDF"}</h3>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 text-sm dark:text-white">
                                    <input
                                        type="text"
                                        placeholder="Exam Session (e.g. Dec 23)"
                                        value={examSession}
