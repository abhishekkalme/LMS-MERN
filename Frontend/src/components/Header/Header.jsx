...
const Header = () => {
  ...
  const primaryLinks = [
    ...
    { name: "Imp. Qs", path: "/important-questions" },
    { name: "PYQs", path: "/pyqs" },
  ];
            ...
            <nav className="flex items-center gap-1 lg:gap-2">
              ...
              >
                <button
                  ...
                  className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 ${showMoreMenu
                      ? "text-indigo-600 bg-indigo-50/50 dark:text-indigo-300 dark:bg-white/5"
                      : "text-gray-600 hover:text-indigo-600 hover:bg-gray-50 dark:text-gray-400 dark:hover:text-white dark:hover:bg-white/5"
                    }`}
                >
                    ...
                    >
                      {secondaryLinks.map((item) => (
                        <NavLink
                          ...
                          to={item.path}
                          className={({ isActive }) =>
                            `flex items-center px-4 py-2 text-sm font-medium transition-colors ${isActive
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-4 py-3 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {primaryLinks.map((item) => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `flex items-center justify-center py-2.5 px-4 rounded-xl text-sm font-semibold transition-all ${isActive
                          ? "text-indigo-600 bg-indigo-50 dark:text-indigo-300 dark:bg-white/5 shadow-sm ring-1 ring-indigo-500/10"
                          : "text-gray-700 hover:text-indigo-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:text-white dark:hover:bg-white/5 border border-gray-100 dark:border-white/5"
                        }`
                      }
                    >
                      {item.name}
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {secondaryLinks.map((item) => (
                    <NavLink
                      ...
                      onClick={() => setIsMenuOpen(false)}
                      className={({ isActive }) =>
                        `flex items-center justify-center py-2 px-3 rounded-lg text-xs font-medium transition-all ${isActive
                          ? "text-indigo-600 bg-indigo-50 dark:text-indigo-300 dark:bg-white/5"
              ...
              <div className="pt-3 border-t border-gray-100 dark:border-white/5 flex flex-col gap-3">
                {/* Theme toggle and Auth */}
                <div className="flex items-center justify-between gap-3">
                  <button
                    onClick={toggleTheme}
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-white/5 dark:hover:bg-white/10 border border-gray-100 dark:border-white/5 transition-all"
                  >
                    {darkMode ? (
                      <BsSunFill className="text-yellow-400 w-4 h-4" />
                    ) : (
                  ...
                  {isLoggedIn ? (
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 border border-red-100 dark:border-red-500/20 text-red-600 dark:text-red-400 transition-all font-semibold text-sm"
                    >
                      Logout
                    </button>
                  ) : (
                    <Link
                      to="/login"
                      onClick={() => setIsMenuOpen(false)}
                      className="flex-1 flex items-center justify-center py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/20 text-sm font-semibold transition-all"
                    >
                      Login
                    </Link>
                  )}
                ...
                {isLoggedIn && (
                  <button
                    ...
                    className="flex items-center gap-3 p-2 rounded-xl bg-gray-50/50 dark:bg-white/5 border border-gray-100 dark:border-white/5"
                  >
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#6366F1] to-[#7C3AED] p-[2px]">
                      <div className="w-full h-full rounded-full bg-white dark:bg-gray-800 flex items-center justify-center text-xs font-bold text-indigo-600 dark:text-indigo-300 overflow-hidden">
                        {user?.avatar ? (
                          <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
                        ) : (
